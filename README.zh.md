...
帮助与报错**跟随系统区域设置**。只有韩文和英文两种。

```bash
LANG=ko_KR.UTF-8 kayatext 文档.hwp     # 韩文
LANG=C           kayatext 文档.hwp     # 英文
KAYATEXT_LANG=en kayatext 文档.hwp     # 无论区域设置一律英文
```

`LANG` 以 `ko` 开头即为韩文，其余为英文。Windows 上通常没有 `LANG`，因此改看
**系统显示语言**。

> 没有做成七种语言是有原因的。终端的通用语是英文；若把报错分成七种，你贴进 issue
> 的那一行我就读不懂了。
...
就用了」在那种场合过不了关。为这些机构准备了**许可证书、合同文件、优先支持**。**功能一项
也不会增加。**

**价格和凭证形式通过协商确定**，因为各机构的使用范围和所需文件各不相同。
请联系 **kayautils@gmail.com**。

**禁止再分发。** 把安装包放在别人能取走的地方也属于此列 —— 公司共享文件夹、文件
服务器、协同办公系统的资料库、网盘、开启了共享的云盘都算。介绍给别人时，请转发本仓库
的地址而不是文件。如果确有在组织内分发的需要，请联系 **kayautils@gmail.com**，
我们可以另行商定条件。
...
```bash
./kayatext --licenses      # 随附的开源组件
./kayatext --terms         # 本程序自身的使用条件
```

发布包中的 `THIRD-PARTY-NOTICES.md` 和 `LICENSE.txt` 也有全文。
